...
class dm(commands.GroupCog):
    ...
    @app_commands.command(description="Claim the role of dungeon master within current category.")
    ...
    @app_commands.command(description="Unclaim dm for category. Admins and the Current DM are the only users able to perform this action.")
    async def unclaim(self, interaction: discord.Interaction):
        try:
            Category, Guild, ID = dice.ctx_info(self, interaction)
            permissions = interaction.channel.permissions_for(interaction.user)            
            if ('administrator', True) in permissions:
                override = True # if the user is an admin, override the check
